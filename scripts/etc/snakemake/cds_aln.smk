...
SPEC_TREE = config["species_tree_file"]
print(SPEC_TREE)
...
FILTERFILE = os.path.join(DATADIR, "aln", "02-Filter-spec7-seq50-site50", "aln-filter-passed-spec7-seq50-site50.txt");
...
rule all:
    input:
        expand(os.path.join(DATADIR, "hyphy", "absrel", "{filtered_locus}-cds.json"), filtered_locus=filtered_loci)
...
#     input: 
#         os.path.join(DATADIR, "aln", "02-Filter-spec7-seq50-site50", "cds", "{filtered_locus}-cds.guidance.filter.fa")
#     output:
...
rule absrel:
    input:
        aln = os.path.join(DATADIR, "aln", "02-Filter-spec7-seq50-site50", "cds-spec", "{filtered_locus}-cds.guidance.filter.fa"),
        tree = SPEC_TREE
    output:
        os.path.join(DATADIR, "hyphy", "absrel", "{filtered_locus}-cds.json")
    log:
        os.path.join(DATADIR, "hyphy", "logs", "absrel", "{filtered_locus}-cds.log")
    shell:
        """
